...
def build_similarity_graph(X, var=1.0, eps=0.0, k=0):
    ...
    # Take the euclidean pairwise distance
    similarities = sd.cdist(X, X)
    # Compute the similarity function
    similarities = np.exp(- similarities ** 2 / (2 * var))
    # Removing self similarity
    similarities[np.eye(n, dtype=bool)] = 0
    ...
    if k == 0:
        ...
        """
        W = np.where(similarities < eps, 0, similarities)
    ...
    if k != 0:
        ...
        """
        for i in range(n):
            idx = np.argsort(similarities[i])[:n-k]
            similarities[i][idx] = 0
        W = np.maximum(similarities, similarities.T)
...
def how_to_choose_epsilon():
    ...
    gen_pam = 1. # to understand the meaning of the parameter, read worst_case_blob in generate_data.py
