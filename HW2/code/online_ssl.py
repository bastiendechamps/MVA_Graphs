...
class IncrementalKCenters:
    ...
    def online_ssl_update_centroids(self, face):
        ...
        if self.centroids.shape[0] >= self.max_num_centroids + 1:
            ...
            """
            c1, c2 = np.unravel_index(np.argmin(self.centroids_distances), self.centroids_distances.shape)
    ...
    def online_ssl_compute_solution(self):
        ...
        """
        num_samples = len(self.centroids)
        l_idx = np.arange(self.n_labeled_faces)
        u_idx = np.arange(self.n_labeled_faces, num_samples)
        
        f_l = np.array([self.Y[l_idx] == i for i in range(1, self.n_labels + 1)]).T
        f = np.zeros((len(self.centroids), self.n_labels))
...
if __name__ == '__main__':
    # create_user_profile('bastien')
    online_face_recognition(['bastien', 'caro'], 15)
