...
def hard_hfs(X, Y, laplacian_regularization, var=1, eps=0, k=0, laplacian_normalization=""):
    ...
    l_idx = np.argwhere(Y > 0).flatten()
    u_idx = np.argwhere(Y == 0).flatten()
    ...
    f_l = np.array([Y[l_idx] == i for i in range(1, num_classes + 1)]).T
    f = np.zeros((num_samples, num_classes))
    
    # Build the regularized Laplacian
    Q = build_laplacian_regularized(X, laplacian_regularization, var, eps, k, laplacian_normalization)
    Q_uu = Q[u_idx, :][:, u_idx]
    W_ul = - Q[u_idx, :][:, l_idx]
    
    # Compute the HFS solution for unlabeled data
    f_u = np.linalg.inv(Q_uu) @ (W_ul @ f_l)
...
def two_moons_hfs():
    ...
    # Shuffle data 
    # shuffle = np.random.rand(num_samples).argsort().flatten()
    # X = X[shuffle, :]
    # Y = Y[shuffle]
    ...
    eps = 0
    k = 10
    laplacian_regularization = 0.001
...
def soft_hfs(X, Y, c_l, c_u, laplacian_regularization, var=1, eps=0, k=0, laplacian_normalization=""):
    ...
    y = np.array([Y == i for i in range(1,  num_classes + 1)]).T
    l_idx = np.argwhere(Y > 0).flatten()
    u_idx = np.argwhere(Y == 0).flatten()
    ...
    f = np.linalg.inv(C_inv @ Q + np.eye(num_samples)) @ y
...
def hard_vs_soft_hfs():
    ...
    print('Hard HFS accuracy:', accuracy[0])
    print('Soft HFS accuracy:', accuracy[1])
